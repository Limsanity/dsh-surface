...
//
// NO bare `@deepseek-ai/dsh-*` imports (a link-installed plugin's host cannot
// resolve them by name). Instead it consumes the LIVE session's own API —
// `session.surface.nodes` and `session.deriveEventMessage(event)` — for the
// surface fold/projection, and uses injected `ctx` services for the rest. The
// only dsh-specific constant re-derived here is the one-line compaction
// checkpoint marker (`isCompactCheckpointSource`).
import { randomUUID } from 'node:crypto'
import { buildSurfaceSeed, findLastCheckpointSeq, snapTurnEnd } from './surface-seed.js'
import { makeEvent } from './emit.js'

/** The compaction-checkpoint marker (mirror of `isCompactCheckpointSource`). */
...
async function readSession(ctx, sessionId) {
  ...
  if (attached) return { session: attached, header: attached.header, events: attached.events, live: true }
...
async function surfaceFork(ctx, { sessionId, messageId, atSeq, increaseTitle = true }) {
  ...
  let ckptSeq = findLastCheckpointSeq(events, isCheckpointSource)
  if (ckptSeq === undefined) {
    ...
    if (agent && compaction?.compactNow) {
      try {
        ...
        ckptSeq = findLastCheckpointSeq(events, isCheckpointSource)
      } catch {
